...
<section class="py-5">
    <div class="container">
        <div class="row">
            ...
            @forelse ($mobil as $item)
                <div class="col-md-4 mb-4">
                    <div class="card h-100 shadow-sm">
                        @if ($item->gambar)
                            <img src="{{ asset('storage/' . $item->gambar) }}" class="mobil-image" alt="Gambar Mobil">
                        @else
                            <div class="d-flex align-items-center justify-content-center bg-light text-muted" style="height: 200px; border-top-left-radius: calc(0.25rem - 1px); border-top-right-radius: calc(0.25rem - 1px);">
                                <small>Tidak ada gambar</small>
                            </div>
                        @endif
                        <div class="card-body d-flex flex-column"> {{-- Added d-flex flex-column here --}}
                            <div>
                            ...
                            <div class="card-footer bg-transparent border-0 d-flex justify-content-between px-0 pb-0 mt-auto"> {{-- Added mt-auto here --}}
                                ...
                                {{-- **PERBAIKAN KODE TOMBOL HEART DIMULAI DI SINI** --}}
                                <form action="{{ route('toggle-favorit', $item->id) }}" method="POST" class="ms-1">
                                    @csrf
                                    <button type="submit" class="btn btn-outline-primary rounded-start-0" {{ Auth::check() && $pesananAktif ? 'disabled' : '' }}>
                                        @auth
                                            {{-- Cek apakah mobil ini sudah difavoritkan oleh user yang login --}}
                                            @if (Auth::user()->favorites->contains($item->id))
                                                <i class="bi bi-heart-fill text-danger"></i> {{-- Hati terisi jika favorit --}}
                                            @else
                                                <i class="bi bi-heart"></i> {{-- Hati kosong jika belum favorit --}}
                                            @endif
                                        @else
                                            <i class="bi bi-heart"></i> {{-- Default hati kosong jika belum login --}}
                                        @endauth
                                    </button>
                                </form>
                                {{-- **PERBAIKAN KODE TOMBOL HEART BERAKHIR DI SINI** --}}
            ...
            @empty
                <div class="col-12">
                    <div class="alert alert-info text-center">Belum ada mobil yang tersedia.</div>
                </div>
            @endforelse
        </div>
    </div>
